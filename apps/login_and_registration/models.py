...
class UserManager(models.Manager):
    ...
    def edit_validation(self, postData):
        ...
        if not EMAIL_REGEX.match(postData['email']):
            errors['email'] = "Please provide a valid email"
        if len(filter_to_check_for_unique_email) > 0 and filter_to_check_for_unique_email[0].email != current_user.email:
            errors['email_not_unique'] = "Email already in use, try again"
        return errors
    ...
    def login_validation(self, postData):
        ...
        elif bcrypt.checkpw(postData['password'].encode(), user[0].p_hash.encode()):
            return errors
        else:
            errors['email_validate'] = "Invalid login Information"
        return errors
